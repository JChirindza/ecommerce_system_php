...
<div class="d-sm-flex align-items-center justify-content-between m-3">
	<h1 class="pageTitle">Meus Pedidos</h1>
	<?php if($_GET['p'] == 'manord') { ?>
		<a type="button" class="btn-primary btn-sm" href="meuspedidos.php?p=add"><i class="fas fa-cart-plus"></i> Adicionar pedido </a>
	<?php } else { ?>
		<a type="button" class="btn-primary btn-sm" href="meuspedidos.php?p=manord"><i class="fas fa-cart-arrow-down"></i> Gerir pedido </a>
	<?php } ?>
</div>
<style type="text/css">
...
<div class="card shadow-sm mb-3">
	<div class="card-header">
		...
		<?php } else if($_GET['p'] == 'editOrd') { ?>
			<i class="fas fa-edit"></i> Itens Pedidos
		<?php } ?>
			...
			<form class="form-horizontal" method="POST" action="php_action/editOrder.php" id="editOrderForm">
			  			...
			  			<tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">			  				
			  				<td style="margin-left:20px;">
			  					<div class="form-group">
			  						...
			  						<select class="form-control" name="productName[]" disabled> id="productName<?php echo $x; ?>" onchange="getProductData(<?php echo $x; ?>)" >
			  							<option value="">~~SELECT~~</option>
			  							<?php
			  							$productSql = "SELECT * FROM product WHERE active = 1 AND status = 1 AND quantity != 0";
			  							$productData = $connect->query($productSql);
									...
									<!-- <td style="padding-left:20px;">
										...
										<td style="padding-left:20px;">
											<div class="form-group">
												<input type="number" disabled name="quantity[]" id="quantity<?php echo $x; ?>" oninput="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control text-center" min="1" value="<?php echo $orderItemData['quantity']; ?>" />
											</div>
										</td>
										<td style="padding-left:20px;">			  					
				...
				<div class="row">
					<div class="col-md-6">
						...
						<div class="form-group">
							<label for="vat" class="col-sm-4 control-label gst"><?php if($data[13] == 2) {echo "IGST 17%";} else echo "IVA 17%"; ?></label>
							<div class="col-sm-8">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" disabled id="paid" name="paid" autocomplete="off" onkeyup="paidAmount()" value="<?php echo $data[9] ?>"  />
							</div>
						</div> <!--/form-group-->			  
						<div class="form-group">
							<!-- <label for="due" class="col-sm-4 control-label">Due Amount:</label>
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select class="form-control" disabled name="paymentType" id="paymentType" >
									<option value="">~~SELECT~~</option>
...
<script src="custom/js/myorder.js"></script>
