...
<div class="d-flex" id="wrapper">
	<div class="container-fluid bg-light m-md-2 m-lg-4">
			...
			<div class="col-sm-4 col-md-3 col-lg-3 bg-white p-3">
				...
				<div class="list-group list-group-flush">
					...
					$sql = "SELECT categories_id, categories_name 
							FROM categories 
							WHERE categories_status = 1 AND (SELECT count(*) AS totalProducts FROM product WHERE categories_id = categories.categories_id AND active = 1)  >= 1 LIMIT 3";
		...
		$sql = "SELECT categories_id, categories_name 
		FROM categories 
		WHERE categories_status = 1 AND (SELECT count(*) AS totalProducts FROM product WHERE categories_id = categories.categories_id AND active = 1)  >= 1";
		$result = $connect->query($sql);
		if($result && $result->num_rows > 0) { 
			foreach($result as $categoryData) {
				?>
